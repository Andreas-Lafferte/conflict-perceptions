...
dequa <- db %>% 
  select(PSCi = psci, Clase=class, Sindicato=union, Top10=z_top10, Corporativismo=corp_all,
         País=country, Ola=wave,
         factor) %>% 
  mutate_if(is.factor, as.numeric) %>% 
  sample_n(size = 1000)

mequa <- lmer(PSCi ~ 1 + Clase + Sindicato + 
                Top10 + Corporativismo +
                Top10*Clase +
                (1 + Clase| País), data = dequa, weights = factor, REML = T)


equatiomatic::extract_eq(mequa, 
                         wrap = T,)
```
